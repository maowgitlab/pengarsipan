            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between my-3">
                        ...
                        <?php endif; ?>
                        <!-- Filter Form -->
                        <form method="get" action="report/cetak_surat_masuk.php" target="_blank" class="d-flex align-items-center" id="filterForm">
                            ...
                            <select name="letter_type" class="form-select me-2" style="width: 150px;" onchange="applyLetterTypeFilter(this)">
                                ...
                                <option value="surat_masuk" <?= isset($_GET['letter_type']) && $_GET['letter_type'] === 'surat_masuk' ? 'selected' : ''; ?>>Surat Masuk</option>
                            ...
                            <div id="period_filter" class="me-2" style="display: <?= isset($_GET['filter_type']) && $_GET['filter_type'] === 'period' ? 'flex' : 'none'; ?>;">
                                <input type="date" name="start_date" class="form-control" style="width: 150px;" value="<?= isset($_GET['start_date']) ? $_GET['start_date'] : ''; ?>">
                                <input type="date" name="end_date" class="form-control ms-1" style="width: 150px;" value="<?= isset($_GET['end_date']) ? $_GET['end_date'] : ''; ?>">
                            </div>
                    ...
                    <table class="table table-hover" id="tabelData">
                        ...
                        <tbody>
                            <?php 
                            $no = 1;
                            $letter_type = isset($_GET['letter_type']) ? $_GET['letter_type'] : 'all';
                            $where_clause = '';
                            if ($letter_type === 'surat_masuk') {
                                $where_clause = "WHERE tb_surat.jenis_surat = 'surat_masuk'";
                            } elseif ($letter_type === 'surat_undangan') {
                                $where_clause = "WHERE tb_surat.jenis_surat = 'surat_undangan'";
                            }
                            $query = "SELECT tb_surat_masuk.*, tb_surat.jenis_surat, tb_surat.id AS id_surat 
                                      FROM tb_surat_masuk 
                                      JOIN tb_surat ON tb_surat_masuk.id_surat = tb_surat.id 
                                      $where_clause 
                            ...
                            foreach ($data as $row) : ?>
                                <tr>
                                    ...
                                    <td><a href="assets/file/masuk/<?= htmlspecialchars($row['file']); ?>" target="_blank" class="btn btn-sm btn-primary"><i class="bi bi-file-earmark-pdf"></i></a></td>
                                    <?php if (isset($_SESSION['jabatan']) && $_SESSION['jabatan'] == 'admin') : ?>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-primary" onclick="editData('<?= $row['id']; ?>', '<?= $row['id_surat']; ?>', '<?= addslashes($row['no_surat']); ?>', '<?= $row['tanggal_masuk']; ?>', '<?= $row['tanggal_terima']; ?>', '<?= addslashes($row['pengirim']); ?>', '<?= addslashes($row['instansi']); ?>', '<?= addslashes($row['perihal']); ?>')" data-bs-toggle="modal" data-bs-target="#modalEdit"><i class="bi bi-pencil-square"></i></button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Form Tambah Surat</h5>
            ...
            <div class="modal-body">
                <form class="row g-3" method="post" enctype="multipart/form-data">
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control" id="no_surat" name="no_surat" readonly value="<?= $faker->numerify('##/MSK/' . date('d') . date('m') . '25'); ?>" required>
                    </div>
                    <div class="col-12">
                        ...
                        <select class="form-select" id="id_surat" name="id_surat" required onchange="toggleInstansiField(this)">
                            ...
                            <?php 
                            $data = mysqli_query($koneksi, "SELECT * FROM tb_surat WHERE jenis_surat IN ('surat_masuk', 'surat_undangan')");
                            if (!$data) {
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="row g-3" method="post" enctype="multipart/form-data">
                    ...
                    <div class="col-12">
                        <label for="tanggal_masuk_edit" class="form-label">Tanggal Masuk</label>
                        <input type="date" class="form-control" id="tanggal_masuk_edit" name="tanggal_masuk_edit" required>
                    </div>
                    <div class="col-12">
                        <label for="tanggal_terima_edit" class="form-label">Tanggal Terima</label>
                        <input type="date" class="form-control" id="tanggal_terima_edit" name="tanggal_terima_edit" required>
                    </div>
                    <div class="col-12">
...
<script>
    function editData(id, id_surat, no_surat, tanggal_masuk, tanggal_terima, pengirim, instansi, perihal) {
        ...
        document.getElementById('tanggal_terima_edit').value = tanggal_terima;
        ...
        // Trigger the instansi field visibility based on the selected jenis_surat
    ...
    function toggleInstansiField(select) {
        ...
        noSuratInput.value = `<?php echo $faker->numerify('##/'); ?>${jenisSurat === 'surat_undangan' ? 'UDG' : 'MSK'}/<?php echo date('d') . date('m') . '25'; ?>`;
    }
    ...
    function toggleInstansiFieldEdit(select) {
        ...
        noSuratInput.value = noSuratInput.value.replace(/\/(MSK|UDG)\//, `/${jenisSurat === 'surat_undangan' ? 'UDG' : 'MSK'}/`);
    }
    ...
    function resetForm() {
        ...
        document.getElementById('no_surat').value = '<?php echo $faker->numerify('##/MSK/' . date('d') . date('m') . '25'); ?>';
    }
...
if (isset($_POST['tambah'])) {
    ...
    $tanggalMasuk = mysqli_real_escape_string($koneksi, $_POST['tanggal_masuk']);
    $tanggalTerima = mysqli_real_escape_string($koneksi, $_POST['tanggal_terima']);
    ...
    if (!move_uploaded_file($tmpFile, "assets/file/masuk/$file")) {
        echo "<script>alert('Gagal mengunggah file.');</script>";
        exit;
    }

    $query = mysqli_query($koneksi, "INSERT INTO tb_surat_masuk (id, id_surat, no_surat, tanggal_masuk, tanggal_terima, pengirim, instansi, perihal, file) VALUES (NULL, '$idSurat', '$noSurat', '$tanggalMasuk', '$tanggalTerima', '$pengirim', '$instansi', '$perihal', '$file')");
...
if (isset($_POST['edit'])) {
    ...
    $tanggalMasuk = mysqli_real_escape_string($koneksi, $_POST['tanggal_masuk_edit']);
    $tanggalTerima = mysqli_real_escape_string($koneksi, $_POST['tanggal_terima_edit']);
    $pengirim = mysqli_real_escape_string($koneksi, $_POST['pengirim_edit']);
    ...
    $data = mysqli_query($koneksi, "SELECT file FROM tb_surat_masuk WHERE id='$id'");
    if (!$data) {
    ...
    if ($file) {
        ...
        if ($fileLama['file']) {
            unlink("assets/file/masuk/" . $fileLama['file']);
        }
        move_uploaded_file($tmpFile, "assets/file/masuk/$file");
        $query = mysqli_query($koneksi, "UPDATE tb_surat_masuk SET no_surat='$noSurat', id_surat='$idSurat', tanggal_masuk='$tanggalMasuk', tanggal_terima='$tanggalTerima', pengirim='$pengirim', instansi='$instansi', perihal='$perihal', file='$file' WHERE id='$id'");
    } else {
        $query = mysqli_query($koneksi, "UPDATE tb_surat_masuk SET no_surat='$noSurat', id_surat='$idSurat', tanggal_masuk='$tanggalMasuk', tanggal_terima='$tanggalTerima', pengirim='$pengirim', instansi='$instansi', perihal='$perihal' WHERE id='$id'");
    }

    if ($query) {
        echo "<script>
                alert('Data Surat Berhasil Diubah');
                window.location.href='?halaman=masuk';
            </script>";
    } else {
...
if (isset($_POST['hapus'])) {
    ...
    if ($file['file']) {
        unlink("assets/file/masuk/" . $file['file']);
    }
