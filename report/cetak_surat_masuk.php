...
<body onload="window.print()">
    ...
    if ($filter_type === 'monthly') {
        ...
        $where_clause .= ($where_clause ? ' AND ' : 'WHERE ') . "MONTH(tb_surat_masuk.tanggal_masuk) = '$month' AND YEAR(tb_surat_masuk.tanggal_masuk) = '$year'";
        $title = "LAPORAN DATA SURAT " . ($letter_type === 'surat_masuk' ? 'MASUK' : ($letter_type === 'surat_undangan' ? 'UNDANGAN' : '')) . " BULAN " . date('F', mktime(0, 0, 0, $month, 1)) . " $year";
        ...
        $where_clause .= ($where_clause ? ' AND ' : 'WHERE ') . "tb_surat_masuk.tanggal_masuk BETWEEN '$start_date' AND '$end_date'";
        $title = "LAPORAN DATA SURAT " . ($letter_type === 'surat_masuk' ? 'MASUK' : ($letter_type === 'surat_undangan' ? 'UNDANGAN' : '')) . " PERIODE " . date('d F Y', strtotime($start_date)) . " - " . date('d F Y', strtotime($end_date));
    } elseif ($filter_type === 'yearly') {
        $year = isset($_GET['year']) ? mysqli_real_escape_string($koneksi, $_GET['year']) : date('Y');
        $where_clause .= ($where_clause ? ' AND ' : 'WHERE ') . "YEAR(tb_surat_masuk.tanggal_masuk) = '$year'";
        $title = "LAPORAN DATA SURAT " . ($letter_type === 'surat_masuk' ? 'MASUK' : ($letter_type === 'surat_undangan' ? 'UNDANGAN' : '')) . " TAHUN $year";
    } else {
        $title = "LAPORAN DATA SURAT " . ($letter_type === 'surat_masuk' ? 'MASUK' : ($letter_type === 'surat_undangan' ? 'UNDANGAN' : ''));
    }
